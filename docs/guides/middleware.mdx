...
`ctx.user` and `ctx.res` remain available as compatibility aliases. Prefer `ctx.subject`, `ctx.policy`, `ctx.credentials`, `ctx.response`, and the root helpers in new code.
