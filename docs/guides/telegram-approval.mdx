...
description: "Gate sensitive Panther tool calls with a simple Telegram approval adapter."
---

Use `@panther/approval-telegram` when a policy needs a human approve/deny step without building a custom UI.

```bash theme={null}
pnpm add @panther/approval-telegram
```

## Add an approval

```ts theme={null}
import { telegramApproval } from "@panther/approval-telegram";
import { group, policy, user } from "@panther/core";
...
import {
  ...
  telegramApproval,
} from "@panther/approval-telegram";
...
`telegramApproval(...)` returns `pending` until the store has an approval decision for the request id. Panther denies the current tool call while the approval is pending and includes safe approval metadata in the policy error.
