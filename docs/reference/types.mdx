...
The resource, prompt, template, and completion methods are optional. Tool-only transports remain valid; Panther returns empty list responses for unsupported list operations and errors for unsupported read/get/complete operations.

## MCP server feature aliases

Panther re-exports typed aliases for the new request and result shapes:
