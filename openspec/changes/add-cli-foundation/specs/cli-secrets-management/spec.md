...
The CLI SHALL provide `panther secrets set` as the primary workflow for storing local encrypted credentials used by Panther projects.

#### Scenario: Set default credential
- **WHEN** a developer runs `panther secrets set github.token`
...
The `panther secrets set` command SHALL support storing credentials for a specific user or group.

#### Scenario: Set user credential
- **WHEN** a developer runs `panther secrets set github.token --user alice`
- **THEN** the CLI stores the credential for user `alice` without printing the raw value

#### Scenario: Set group credential
- **WHEN** a developer runs `panther secrets set github.token --group support`
- **THEN** the CLI stores the credential for group `support` without printing the raw value

### Requirement: Local auth compatibility
The secrets workflow SHALL use the same encrypted local auth storage format supported by `PantherAuth.local`.

#### Scenario: Proxy resolves stored secret
- **WHEN** a developer stores a credential with `panther secrets set`
- **THEN** a project using `PantherAuth.local` can resolve that credential according to existing Panther credential precedence

### Requirement: Safe secret output
The CLI SHALL redact secret values in all normal terminal output.

#### Scenario: Secret stored
- **WHEN** `panther secrets set github.token` succeeds
...
- **THEN** the CLI still supports creating Panther local auth files compatible with current projects
