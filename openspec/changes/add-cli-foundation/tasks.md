...
- [x] 1.3 Preserve existing `panther auth ...` behavior or map it to compatible internal auth/secrets operations
...
- [x] 2.1 Add the default TypeScript project template files for `package.json`, `tsconfig.json`, Panther project config, `.env.example`, `.gitignore`, and `src/index.ts`
...
- [x] 2.5 Initialize local Panther auth files through existing `PantherAuth.local`-compatible encrypted credential storage

## 3. Init Command

- [x] 3.1 Implement `panther init [project-name]` project name resolution and prompt when omitted
...
- [x] 3.8 Print final setup output with `cd <project>` and `panther dev` next steps

## 4. Health Commands

- [x] 4.1 Implement project discovery for project-scoped commands from the current directory or ancestors
- [x] 4.2 Implement `panther doctor` environment checks for Node version, package managers, git, Docker warning readiness, ports, and CLI writable directories
- [x] 4.3 Implement `panther doctor --fix` with one approval prompt per supported fix in deterministic order
- [x] 4.4 Implement `panther check` static project validation for config, expected files, package metadata, auth references, and proxy entrypoint
- [x] 4.5 Implement MCP connectivity and tool discovery checks for `panther check` where enabled
- [x] 4.6 Implement `panther check --offline` and `panther check --strict`
- [x] 4.7 Ensure health commands report mixed pass/warn/fail results with clear grouped output and exit codes

## 5. Runtime Commands

- [x] 5.1 Implement `panther dev` to run the discovered Panther project locally through the selected package manager or configured entrypoint
- [x] 5.2 Add clear failure behavior for `panther dev` outside a Panther project
- [x] 5.3 Implement `panther build` to validate the project and write a deterministic local artifact under `.panther/build`
- [x] 5.4 Print build output directory and runtime entrypoint after a successful build
- [x] 5.5 Add placeholder behavior for `panther deploy` that clearly reports deploy is not available yet and points to `panther build`

## 6. Secrets Command

- [x] 6.1 Implement `panther secrets set <reference>` with secure value prompting and default credential storage
- [x] 6.2 Add `--user` and `--group` support for subject-scoped credentials
- [x] 6.3 Ensure all secret command output redacts raw values
- [x] 6.4 Verify `panther secrets set` writes storage compatible with `PantherAuth.local` credential resolution
...
- [x] 7.5 Add tests for `secrets set` redaction and PantherAuth-compatible storage
- [x] 7.6 Update getting-started docs so `panther init` and `panther dev` become the recommended quickstart path
