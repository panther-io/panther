...
- **THEN** Panther executes it for matching operations and continues to the next handler

#### Scenario: New middleware short-circuits
- **WHEN** new middleware returns a tool result or denial response
- **THEN** Panther stops the remaining route pipeline and returns that response to the MCP client
...
- **THEN** Panther runs the handler before forwarding the call upstream

#### Scenario: Server wildcard matches
- **WHEN** `proxy.tool("github.*", handler)` is registered and any GitHub tool is called
- **THEN** Panther runs the handler for that GitHub tool call
...
- **THEN** Panther runs the handler for each matching call
...
- **THEN** Panther runs the handler only for operations scoped to the GitHub upstream server
...
- **THEN** Panther runs the handler for that tool call
...
- **THEN** Panther executes the matching handlers in that registration order for a matching tool call

#### Scenario: Multiple tool handlers compose
- **WHEN** multiple matching tool handlers call `next()`
- **THEN** Panther runs each handler once and then forwards the call upstream
...
- **THEN** Panther does not execute the tool route handler for the list operation
...
- **THEN** Panther returns the policy denial without relying on `server.tool(..., requireGroup(...))` style runtime authorization
...
- **THEN** Panther runs the route validation before forwarding the allowed call upstream
