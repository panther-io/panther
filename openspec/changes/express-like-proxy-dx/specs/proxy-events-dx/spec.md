...
- **THEN** Panther invokes the handler when a downstream MCP session starts

#### Scenario: Proxy registers filtered tool event
- **WHEN** an application registers `proxy.on("tool:success", { server: "github" }, handler)`
- **THEN** Panther invokes the handler only for successful tool calls on the GitHub server
...
- **THEN** Panther invokes the handler for GitHub tool errors and not for other servers
...
- **WHEN** Panther begins processing an allowed tool call
- **THEN** Panther emits `tool:start` with context and timing metadata

#### Scenario: Tool success event
- **WHEN** an upstream tool call completes successfully
- **THEN** Panther emits `tool:success` with context, result, and duration metadata

#### Scenario: Tool error event
- **WHEN** a tool call fails before a successful result is returned
- **THEN** Panther emits `tool:error` with context, normalized error, and duration metadata

#### Scenario: Tool after event
- **WHEN** a tool call completes with either success or failure
- **THEN** Panther emits `tool:after` with context and completion metadata
...
- **THEN** Panther uses the returned tools as the list response
...
- **THEN** Panther emits `session:start` with transport and subject context

#### Scenario: Session ends
- **WHEN** a downstream MCP session ends or is cleaned up
- **THEN** Panther emits `session:end` with transport, subject, and duration metadata where available
