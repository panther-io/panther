...
- **THEN** the proxied resource preserves that metadata except for routing fields intentionally rewritten by Panther
...
- **THEN** Panther calls `resources/read` on the owning upstream with the original URI

#### Scenario: Unknown resource URI
- **WHEN** a downstream client reads a URI that Panther cannot route
- **THEN** Panther returns an MCP error instead of calling an arbitrary upstream

#### Scenario: Resource contents preserved
- **WHEN** an upstream returns text or blob resource contents
- **THEN** Panther returns equivalent contents to the downstream client with proxied content URIs where routing requires it
...
- **THEN** Panther can recover the owning upstream server and original URI template from the downstream reference
...
- **THEN** Panther calls `prompts/get` on the owning upstream with the original prompt name and same arguments

#### Scenario: Unknown prompt name
- **WHEN** a downstream client gets a prompt name that Panther cannot route
- **THEN** Panther returns an MCP invalid params error

#### Scenario: Embedded resources in prompt messages
- **WHEN** an upstream prompt response contains embedded resource content
- **THEN** Panther preserves the embedded content payload for the downstream client
...
- **THEN** Panther forwards completion to the owning upstream with the original prompt reference
...
- **THEN** Panther forwards completion to the owning upstream with the original resource template reference

#### Scenario: Completion result preserved
- **WHEN** an upstream returns completion values, total, and hasMore
- **THEN** Panther returns the same completion result to the downstream client
...
- **THEN** Panther continues to proxy that transport's tools without requiring resource, prompt, or completion methods
