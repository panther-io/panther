...
The system SHALL provide a native MCP Streamable HTTP upstream transport that connects Panther to MCP servers over `http://` and `https://` URLs using MCP protocol semantics.

#### Scenario: List tools through native HTTP upstream
- **WHEN** Panther lists tools for a server configured with the native Streamable HTTP upstream transport
...
- **WHEN** Panther calls a tool for a server configured with an `https://` Streamable HTTP upstream URL
...
- **WHEN** Panther closes a server configured with the native Streamable HTTP upstream transport
- **THEN** the transport closes active MCP sessions and releases protocol resources

### Requirement: Native SSE upstream transport
The system SHALL provide a native MCP SSE upstream transport that connects Panther to SSE-capable MCP servers and manages the event-stream lifecycle.

#### Scenario: List tools through SSE upstream
- **WHEN** Panther lists tools for a server configured with the SSE upstream transport
...
- **WHEN** Panther calls a tool for a server configured with the SSE upstream transport
...
- **WHEN** Panther closes a server configured with the SSE upstream transport
...
- **WHEN** a stdio upstream server is configured with `McpServer.env(user)` and Panther receives calls from distinct users
- **THEN** Panther creates or reuses env-aware stdio transports keyed by user identity with the resolved environment values

#### Scenario: Per-user stdio isolation
- **WHEN** a stdio upstream server is configured with an isolation runtime
- **THEN** Panther queues calls according to the isolation runtime using the resolved user identity
...
- **THEN** Panther uses MCP initialization, session handling, and MCP request semantics rather than fixed `/listTools` and `/callTool` REST endpoints
